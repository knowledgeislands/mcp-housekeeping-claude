 ...
 *
 * The fail-safe defaults unannotated tools to the most-restricted level so
 * that forgetting to annotate a new tool excludes it from the default `read`
 * gate rather than silently allowing it through.
 *
 * Housekeeping's destructive tools also default `dry_run: true` — the gate
 * controls *visibility*, `dry_run` controls *effect*. Both layers must hold
 * for new destructive tools (see CLAUDE.md → Security Requirements).
 */
